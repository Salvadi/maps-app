...
const EDGE_FN_URL = `${process.env.REACT_APP_SUPABASE_URL}/functions/v1/cert-search`;
  ...
  const handleGenerateReport = useCallback(() => {
    ...
    try {
      const citationsHtml = response.citations.map(c =>
        `<li><strong>[${c.index}]</strong> ${c.certName} — ${c.section}${c.content ? `<br><small>${c.content}</small>` : ''}</li>`
      ).join('');

      const resultsHtml = response.results.map((r, i) =>
        `<div style="margin-bottom:12px;padding:10px;border:1px solid #ddd;border-radius:6px;">
          <strong>[${i + 1}] ${r.certName}</strong>${r.section ? ` — ${r.section}` : ''}
          ${r.hasTable ? ' <span style="background:#e0f2fe;padding:2px 6px;border-radius:4px;font-size:12px;">Tabella</span>' : ''}
          <br><small>Rilevanza: ${(r.score * 100).toFixed(0)}%</small>
          <pre style="white-space:pre-wrap;font-size:12px;margin-top:8px;">${r.content}</pre>
        </div>`
      ).join('');

      const html = `<!DOCTYPE html>
<html lang="it"><head><meta charset="UTF-8">
<title>Report Tecnico — ${response.query}</title>
<style>body{font-family:Arial,sans-serif;max-width:800px;margin:40px auto;padding:20px;color:#222;}
h1{font-size:20px;}h2{font-size:16px;margin-top:24px;}
ul{padding-left:20px;}pre{background:#f5f5f5;padding:8px;border-radius:4px;overflow-x:auto;}
@media print{body{margin:0;padding:16px;}}</style></head>
<body>
<h1>Report Tecnico Antincendio</h1>
<p><strong>Query:</strong> ${response.query}</p>
<h2>Risposta AI</h2>
<p style="white-space:pre-wrap;">${response.answer}</p>
${citationsHtml ? `<h2>Fonti Citate</h2><ul>${citationsHtml}</ul>` : ''}

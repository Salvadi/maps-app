...
// Read the JSON file
const mappings = JSON.parse(fs.readFileSync('docs/importazione bulk data/mappings-piani8-6-cavi.json', 'utf8'));
...
// Generate SQL
let sql = `-- Import mapping entries for Piani 8, 7, 6 - CAVI (INTERVENTO 2)
-- Project ID: ${projectId}
-- User ID: ${userId}
-- Total entries: ${mappings.length}
-- Generated at: ${new Date().toISOString()}
--
-- IMPORTANTE: Tutte queste entries hanno INTERVENTO = 2
--
...
Object.keys(byFloor).sort().reverse().forEach(floor => {
  ...
  sql += `-- ═══════════════════════════════════════════════════════════
-- PIANO ${floor} (${floorMappings.length} entries) - INTERVENTO 2
-- ═══════════════════════════════════════════════════════════
...
// Write to file
fs.writeFileSync('docs/importazione bulk data/import-piani8-6-cavi.sql', sql);

console.log('✅ SQL generato con successo!');
console.log(`📄 File: import-piani8-6-cavi.sql`);
console.log(`📊 ${mappings.length} INSERT statements generati`);
console.log('\n⚠️  IMPORTANTE: Tutte le entries hanno INTERVENTO = 2');
console.log('\n📋 Breakdown per piano:');
Object.keys(byFloor).sort().reverse().forEach(floor => {
  const count = byFloor[floor].length;
  console.log(`   Piano ${floor}: ${count} entries (tutte con Cavi/Corrugati + Fascio di cavi)`);
});
console.log('\n🚀 Prossimi passi:');
console.log('   1. Apri il file import-piani8-6-cavi.sql');
console.log('   2. Esegui lo script SQL su Supabase');
console.log('   3. Verifica che tutte le entries siano state importate');
console.log('   4. Controlla che abbiano INTERVENTO = 2');

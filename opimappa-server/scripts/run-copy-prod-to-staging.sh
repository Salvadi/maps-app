#!/usr/bin/env bash
# Runner one-off: copia PROD -> STAGING (dropdown, products, progetto CANTIERE PROVA + storage).
# Uso (da Windows PowerShell):
#   Get-Content -Raw opimappa-server/scripts/run-copy-prod-to-staging.sh | ssh levi@100.111.232.12 'bash -s'
set -euo pipefail
echo "=== Runner copy PROD -> STAGING ==="
WORK=/home/levi/copy-pts
mkdir -p "$WORK"
cat > "$WORK/copy-prod-to-staging.ts" <<'OPI_TS_EOF'
/**
 ...
 *      MinIO prod -> minio-staging (HeadObject skip se gia presente).
 *
...
async function main() {
  ...
  if (found.length > 1) console.warn(`  Attenzione: ${found.length} progetti con questo titolo, uso il piu vecchio.`);
...
req(){ local v; v=$(docker exec "$1" printenv "$2" 2>/dev/null || true); if [ -z "$v" ]; then echo "MANCA env $2 nel container $1" >&2; exit 1; fi; printf '%s' "$v"; }

echo "Derivo connessioni dai container in esecuzione..."
SRC_DB_URL=$(req opimappa-api DATABASE_URL)
DST_DB_URL=$(req opimappa-api-staging DATABASE_URL)
SRC_MINIO_ENDPOINT=$(req opimappa-api MINIO_ENDPOINT)
SRC_MINIO_ACCESS_KEY=$(req opimappa-api MINIO_API_ACCESS_KEY)
SRC_MINIO_SECRET_KEY=$(req opimappa-api MINIO_API_SECRET_KEY)
DST_MINIO_ENDPOINT=$(req opimappa-api-staging MINIO_ENDPOINT)
DST_MINIO_ACCESS_KEY=$(req opimappa-api-staging MINIO_API_ACCESS_KEY)
DST_MINIO_SECRET_KEY=$(req opimappa-api-staging MINIO_API_SECRET_KEY)
...
SRC_MINIO_ENDPOINT=$SRC_MINIO_ENDPOINT
SRC_MINIO_ACCESS_KEY=$SRC_MINIO_ACCESS_KEY
SRC_MINIO_SECRET_KEY=$SRC_MINIO_SECRET_KEY
DST_MINIO_ENDPOINT=$DST_MINIO_ENDPOINT
DST_MINIO_ACCESS_KEY=$DST_MINIO_ACCESS_KEY
DST_MINIO_SECRET_KEY=$DST_MINIO_SECRET_KEY
PROJECT_TITLE=${PROJECT_TITLE:-CANTIERE PROVA}
EOF2

echo "Avvio container one-off (node:22-alpine)..."
set +e
docker run --rm --network "$NET" --env-file "$WORK/.env" -v "$WORK":/work node:22-alpine \
  sh -c 'cd /work && npm i --silent postgres @aws-sdk/client-s3 tsx >/tmp/npm.log 2>&1 && npx tsx copy-prod-to-staging.ts'
code=$?
set -e

shred -u "$WORK/.env" 2>/dev/null || rm -f "$WORK/.env"
echo "Env-file rimosso. Exit=$code"
exit $code
